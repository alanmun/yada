...
# Sizes generated by scripts/make_icons.py.
...
def _tick_url(indicator: int) -> str:
    ...
    Qt stylesheets cannot draw a checkmark and do not accept data URIs, so it has to be a
    real file. Leaving the indicator unstyled instead produced a near-black tick on the
    dark palette -- invisible.
    """
    best = min(_TICK_SIZES, key=lambda size: abs(size - indicator))
...
/* The language picker's own checkboxes. Styled explicitly for the same reason: the
   native tick was drawn near-black on a dark popup and could not be seen. */
